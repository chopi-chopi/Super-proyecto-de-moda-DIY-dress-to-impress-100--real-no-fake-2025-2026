...
def nuevo_juego():
    ...
    # mismo pero shorts
    small_font = pygame.font.Font(os.path.normpath(os.path.join(os.path.dirname(__file__), "..", "assets", "daydream_3", "Daydream DEMO.otf")), 20)
    change_bottom_img = None
    try:
        change_bottom_img = pygame.image.load(os.path.join(base_dir, "..", "assets", "BOTONES-interactivos","BUTTONS UI", "3- bottoms.png")).convert_alpha()
        change_bottom_img = pygame.transform.smoothscale(change_bottom_img, (40, 40))
    except Exception as e:
        print(f"Error cargando la imagen: {e}")
    ...
    def load_page():
        ...
        cloth_paths_page = []  # lista d lista
        thumb_paths = []
        thumbs = []
                    ...
                    if change_body_button.checkForInput(event.pos):
                        ...
                        for idx, rect in enumerate(thumb_rects):
                            if rect.collidepoint(event.pos) and idx < len(thumbs):
                                ...
                                # Posiciones de los botones toggle
                                toggle_buttons = []
